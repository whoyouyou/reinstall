...
realm	amd64	zhboner/realm	^realm-x86_64-unknown-linux-musl[.]tar[.]gz$
realm	arm64	zhboner/realm	^realm-aarch64-unknown-linux-musl[.]tar[.]gz$
sui	amd64	alireza0/s-ui	^s-ui-linux-amd64[.]tar[.]gz$
sui	arm64	alireza0/s-ui	^s-ui-linux-arm64[.]tar[.]gz$
...
setup_base() {
    ...
    apt_install ca-certificates curl openssl tar iproute2 systemd-timesyncd jq
...
setup_bbr() {
    ...
    local ram max_buf=16777216
    ram=$(awk '/^MemTotal:/ {print int($2/1024)}' /proc/meminfo 2>/dev/null || echo 512)
    # 可见内存 >= 800MiB 使用 32MiB 最大 TCP 缓冲；更小内存使用 16MiB，兼顾高 RTT 链路与内存占用。
    if ((ram >= 800)); then
        max_buf=33554432
    fi
...
# TCP 长肥管道自适应缓冲：提高高 RTT / 高带宽链路可用窗口上限，不承诺特定线路吞吐。
...
# 系统总文件句柄容量。不要覆盖 fs.nr_open，避免把系统原本更高的每进程上限反向降低。
fs.file-max = 1048576
EOF
    sysctl -p /etc/sysctl.d/90-fleet-bbr.conf

    # 交互登录会话的 nofile 上限；高并发 systemd 服务另外在各自 unit 中单独设置。
    mkdir -p /etc/security/limits.d
    cat >/etc/security/limits.d/90-fleet-limits.conf <<'EOF'
* soft nofile 65535
* hard nofile 524288
...
EOF
    ...
    # 清理旧版可能留下的全局 systemd DefaultLimitNOFILE 覆盖，避免无关服务继承高上限。
    rm -f /etc/systemd/system.conf.d/50-fleet-limits.conf
...
get_asset() (
    local app=$1 out=$2 arch record repo asset_regex api tmp_json tag asset_name url digest expected actual
    local -a family_args=()
    arch=$(dpkg --print-architecture)
    record=$(awk -F '\t' -v a="$app" -v b="$arch" '$1==a && $2==b {print $3 "\t" $4}' "$BASE/assets.tsv")
    [[ -n $record && $record != *$'\n'* ]] || { echo "$app/$arch has no release asset mapping" >&2; return 1; }
    IFS=$'\t' read -r repo asset_regex <<<"$record"
    [[ $repo =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ && -n $asset_regex ]] || return 1
    ...
    command -v jq >/dev/null 2>&1 || { echo 'jq is required for verified GitHub Release metadata' >&2; return 1; }
    api="https://api.github.com/repos/$repo/releases/latest"
    tmp_json=$(mktemp "$STATE/${app}-release.XXXXXXXX.json")
    rm -f -- "$out.part"
    trap 'rm -f -- "${tmp_json:-}" "${out:-}.part"' EXIT
    ...
    curl "${family_args[@]}" --fail --silent --show-error --location --proto '=https' --proto-redir '=https' \
        ...
        --connect-timeout 15 --max-time 120 --retry 3 --retry-delay 3 -o "$tmp_json" "$api"
    [[ -s $tmp_json ]] || { echo "empty GitHub release metadata for $app" >&2; return 1; }

    # /releases/latest is expected to be a published full release; verify that contract again and require exactly one matching asset.
    mapfile -t _asset_fields < <(jq -r --arg re "$asset_regex" '
        select(.draft == false and .prerelease == false)
        | .tag_name as $tag
        | [.assets[] | select(.state == "uploaded" and (.name | test($re)))] as $m
        | select(($m | length) == 1)
        | $m[0] as $a
        | $tag, $a.name, $a.browser_download_url, ($a.digest // "")
    ' "$tmp_json")
    [[ ${#_asset_fields[@]} -eq 4 ]] || { echo "latest stable $app release has zero or multiple matching assets" >&2; return 1; }
    tag=${_asset_fields[0]}
    asset_name=${_asset_fields[1]}
    url=${_asset_fields[2]}
    digest=${_asset_fields[3]}
    unset _asset_fields

    [[ -n $tag && $asset_name != */* ]] || return 1
    [[ $url == https://github.com/$repo/releases/download/* ]] || { echo "unexpected asset URL for $app: $url" >&2; return 1; }
    [[ $digest =~ ^sha256:[0-9a-fA-F]{64}$ ]] || {
        echo "GitHub did not publish a SHA256 digest for $app $tag/$asset_name; refusing unverified install" >&2
        return 1
    }
...
通用 Debian 重装封装 / Fleet Reinstall
  ...
  bash reinstall.sh reset                      撤销已准备的重装启动项
...
while (($#)); do
    case "$1" in
        -h|--help) usage; exit 0 ;;
        reset) exec bash "$BASE/vendor/reinstall.sh" reset ;;
        debian|Debian) ;;
...
if [[ $AUTO_REBOOT == yes ]]; then
    ...
    printf '取消本次准备：重新运行原始单文件脚本：bash reinstall.sh reset\n'
fi
...
143cf683944cba6ec689d55aa1dd769931e5669e7d33794ea507b09843916ddb  payload/assets.tsv
a8fc7ee559f49e75e0e1ebd319e4c97ebc4e7ce5144d49adf65bb67cdc2bbf07  payload/firstboot.sh
...
c7509226439c0c218329b668bc4c9bb3faf8d7c0973f07193d84214a6612755c  reinstall.sh
...
cleanup_unpack() { rm -rf -- "$FLEET_UNPACK"; }
trap cleanup_unpack EXIT
set +e
bash "$FLEET_UNPACK/reinstall.sh" "$@"
rc=$?
set -e
exit "$rc"
